...
describe('End to end', async () => {
  ...
  it(`Test basic csv read with skipLines=1`, async () => {
    let [header, output] = await new Promise((resolve, reject) => {
      let inputStream = Fs.createReadStream(Path.join(__dirname, 'test-header.csv'), 'utf8');
      let header = null;
      let output = [];
  ...
  it(`Test basic csv read with skipLines=2`, async () => {
    let [header, output] = await new Promise((resolve, reject) => {
      let inputStream = Fs.createReadStream(Path.join(__dirname, 'test-header.csv'), 'utf8');
      let header = null;
      let output = [];
  ...
  it(`Test basic csv read with parsed numbers and booleans`, async () => {
    ...
    assert.deepStrictEqual(output, [
      ...
      NaN, -NaN,
      Infinity, -Infinity,
      2.343240e+6, -2.343240e+6, // eslint-disable-line no-loss-of-precision
      true, false,
    ]);
  });
